...
具体来说，智能体的目标是寻找一个最优策略 [ \\pi\^\*]{.ztext-math
eeimg="1" tex=" \\pi^*"}
...
data-original="https://pica.zhimg.com/v2-25e2a31c9d52d9a6c59018346f99c304_r.jpg"
data-original-token="v2-25e2a31c9d52d9a6c59018346f99c304"
data-rawheight="434" data-rawwidth="720" data-size="normal"
width="720" />
...
3\. **状态转移（Transition Dynamics, P）**：从当前状态 [s ]{.ztext-math
eeimg="1" tex="s "} 采取动作 [a]{.ztext-math eeimg="1" tex="a"}
后，会以一定的概率转移到下一个状态 [s{\\prime}]{.ztext-math eeimg="1"
tex="s{\\prime}"} 。

4\. **奖励（Reward,
R）**：智能体在每个时刻（或每次转移）获得的即时回报，反映了该动作在此状态下的好坏。

5\. **折扣因子（Discount Factor,** [\\gamma]{.ztext-math eeimg="1"
tex="\\gamma"} **）**：用于平衡当前奖励和未来奖励的重要性，数值通常在
(0, 1\] 之间。

6\.
**轨迹（Trajectory）：**在强化学习中，智能体从环境的初始状态开始，与环境交互直至到达终止状态所经历的一系列状态、动作以及相应的奖励就构成了一条完整的"轨迹"。形式上可以表示为：
[\\tau = \\{s_0, a_0, r_1, s_1, a_1, r_2, \\dots, s\_{T-1}, a\_{T-1},
r_T, s_T\\}]{.ztext-math eeimg="1"
tex="\\tau =  \\{s_0, a_0, r_1, s_1, a_1, r_2, \\dots, s_{T-1}, a_{T-1}, r_T, s_T\\}"}
。其中， [T]{.ztext-math eeimg="1" tex="T"}
表示回合（Episode）结束的时间步。在某些不定长的任务中， [T ]{.ztext-math
eeimg="1" tex="T "}
可能是一个随机值。轨迹可以帮助我们理解智能体如何从起始状态一步步演化到最终状态，并获取相应的奖励序列。

7\.
**经验（Experience）：**智能体可与环境交互多次，进行多次实验，形成多个轨迹。多个轨迹的集合被称为经验。即
[E = \\{\\tau_1, \\tau_2, \..., \\tau_K \\}]{.ztext-math eeimg="1"
tex="E = \\{\\tau_1, \\tau_2, ..., \\tau_K \\}"} 。

8\.
**回报（Return）：**强化学习的目标是学习一个好的策略，智能体按照这样的策略和环境交互，让累积奖励达到最大。**这个累积奖励就是回报**。通常指从某一个时间步
t 开始，对未来所有奖励的加总，常用符号 [G_t]{.ztext-math eeimg="1"
tex="G_t"} 表示（有时也用 [R]{.ztext-math eeimg="1" tex="R"}
表示）。其定义为

[G_t = \\sum\_{t=0}\^{T} r\_{t}]{.ztext-math eeimg="1"
tex="G_t = \\sum_{t=0}^{T} r_{t}"}

通常现在的奖励和将来的奖励权重是不同的，比如今天奖励1万元和50年后奖励1万元，两者的价值大概率不同。因此会加上折扣因子[\\gamma]{.ztext-math
eeimg="1" tex="\\gamma"}。若存在折扣因子，则回报可以写为：

[G_t = r\_{0} + \\gamma r\_{1} + \\gamma\^2 r\_{2} + \\dots +
\\gamma\^{T} r_T = \\sum\_{t=0}\^{T} \\gamma\^t r_t]{.ztext-math
eeimg="1"
tex="G_t = r_{0} + \\gamma r_{1} + \\gamma^2 r_{2} + \\dots + \\gamma^{T} r_T = \\sum_{t=0}^{T} \\gamma^t r_t"}

其中 [\\gamma \\in (0, 1\]]{.ztext-math eeimg="1"
tex="\\gamma \\in (0, 1]"}
用于平衡当前奖励和未来奖励的重要性。如果没有折扣（ [\\gamma =
1]{.ztext-math eeimg="1" tex="\\gamma = 1"} ），那么回报就是从时间步
[t]{.ztext-math eeimg="1" tex="t"}
开始直到回合结束所获得的总奖励。在有了回报概念后，我们知道，**强化学习的目标是最大化期望回报**。

9\. **价值函数（Value
Function）**：**价值函数衡量的是"期望回报"**。具体分为状态价值函数和动作价值函数。

- - **状态价值函数** [V\^\\pi(s)]{.ztext-math eeimg="1" tex="V^\\pi(s)"}
    表示在状态 s 下，后续按照策略 [\\pi]{.ztext-math eeimg="1"
    tex="\\pi"} 行动所能获得的**期望回报**：\
    [V\^\\pi(s) = \\mathbb{E}\[\\,G_t \\mid s_t = s\\,\].]{.ztext-math
    eeimg="1" tex="V^\\pi(s) = \\mathbb{E}[\\,G_t \\mid s_t = s\\,]."}\
  - **动作价值函数** [Q\^\\pi(s, a)]{.ztext-math eeimg="1"
    tex="Q^\\pi(s, a)"} 表示在状态 s 下执行动作 a，并在之后按照策略 [
    \\pi]{.ztext-math eeimg="1" tex=" \\pi"}
    行动所能获得的**期望回报**：\
    [Q\^\\pi(s, a) = \\mathbb{E}\[\\,G_t \\mid s_t = s, a_t =
    a\\,\].]{.ztext-math eeimg="1"
    tex="Q^\\pi(s, a) = \\mathbb{E}[\\,G_t \\mid s_t = s, a_t = a\\,]."}
...
[ A\^\\pi(s, a) \\;=\\; Q\^\\pi(s, a) \\;-\\; V\^\\pi(s).]{.ztext-math
eeimg="1" tex=" A^\\pi(s, a) \\;=\\; Q^\\pi(s, a) \\;-\\; V^\\pi(s)."}

- - 如果 [A\^\\pi(s, a)]{.ztext-math eeimg="1" tex="A^\\pi(s, a)"} 大于
    0，说明在状态 s 下执行动作 a 要比该状态的平均策略价值要好。\
  - 如果 [A\^\\pi(s, a)]{.ztext-math eeimg="1" tex="A^\\pi(s, a)"} 小于
    0，则说明这个动作比"平均"水平要差。
...
- - **状态价值函数**：\
    [V\^\\pi(s) = \\mathbb{E}\_\\pi\[G_t \\mid s_t = s\],]{.ztext-math
    eeimg="1" tex="V^\\pi(s) = \\mathbb{E}_\\pi[G_t \\mid s_t = s],"}\
    即"在状态 s 下持续按照策略 [ \\pi]{.ztext-math eeimg="1"
    tex=" \\pi"} 行动所能获得的期望回报"。\
  - **动作价值函数**：\
    [ Q\^\\pi(s,a) = \\mathbb{E}\_\\pi\[G_t \\mid s_t = s,\\; a_t =
    a\],]{.ztext-math eeimg="1"
    tex=" Q^\\pi(s,a) = \\mathbb{E}_\\pi[G_t \\mid s_t = s,\\; a_t = a],"}\
    即"在状态 s 下先执行动作 a，然后继续按照策略 [ \\pi ]{.ztext-math
    eeimg="1" tex=" \\pi "} 行动所能获得的期望回报"。\

**贝尔曼方程的自洽性（递归性）**\
价值函数可以通过对下一步状态（或动作）的价值进行加权求期望而"自我定义"：

- - 对状态价值 [V\^\\pi(s)]{.ztext-math eeimg="1" tex="V^\\pi(s)"}
    来说，状态 s 的价值是"执行任一动作后的即时奖励 +
    折扣后下一状态价值"的期望：\
    [ V\^\\pi(s) = \\mathbb{E}{a\\sim\\pi(\\cdot\|s)}\\Bigl\[\\,r(s,a) +
    \\gamma \\,\\mathbb{E}{s{\\prime}\\sim
    P(\\cdot\|s,a)}\[V\^\\pi(s{\\prime})\] \\Bigr\].]{.ztext-math
    eeimg="1"
    tex=" V^\\pi(s) = \\mathbb{E}{a\\sim\\pi(\\cdot|s)}\\Bigl[\\,r(s,a) + \\gamma \\,\\mathbb{E}{s{\\prime}\\sim P(\\cdot|s,a)}[V^\\pi(s{\\prime})] \\Bigr]."}\
  - 对动作价值 [Q\^\\pi(s,a)]{.ztext-math eeimg="1" tex="Q^\\pi(s,a)"}
    来说，则是"该状态动作得到的即时奖励 +
    折扣后下一状态的动作价值"的期望：\
    [Q\^\\pi(s,a) = \\mathbb{E}{s{\\prime}\\sim
    P(\\cdot\|s,a)}\\Bigl\[\\,r(s,a) + \\gamma
    \\,\\mathbb{E}{a{\\prime}\\sim\\pi(\\cdot\|s{\\prime})}\[Q\^\\pi(s{\\prime},a{\\prime})\]
    \\Bigr\].]{.ztext-math eeimg="1"
    tex="Q^\\pi(s,a) = \\mathbb{E}{s{\\prime}\\sim P(\\cdot|s,a)}\\Bigl[\\,r(s,a) + \\gamma \\,\\mathbb{E}{a{\\prime}\\sim\\pi(\\cdot|s{\\prime})}[Q^\\pi(s{\\prime},a{\\prime})] \\Bigr]."}
...
Q(s,a)\\bigr\],]{.ztext-math eeimg="1"
tex="Q(s,a) \\;\\leftarrow\\; Q(s,a) \\;+\\; \\alpha \\bigl[\\,r + \\gamma\\,\\max_{a{\\prime}}Q(s{\\prime},a{\\prime}) \\;-\\; Q(s,a)\\bigr],"}\
...
[A\^{\\pi}(s, a) = Q\^{\\pi}(s, a) - V\^{\\pi}(s)]{.ztext-math eeimg="1"
...
[\\lambda]{.ztext-math eeimg="1" tex="\\lambda"}
...
\\delta\_{t+l}]{.ztext-math eeimg="1"
tex="A_t^{\\lambda} = \\sum_{l=0}^{\\infty} (\\gamma \\lambda)^l \\delta_{t+l}"}

\

其中， [\\delta\_{t+l}]{.ztext-math eeimg="1" tex="\\delta_{t+l}"}
表示每一步的TD误差， [\\gamma]{.ztext-math eeimg="1" tex="\\gamma"}
是折扣因子， [\\lambda]{.ztext-math eeimg="1" tex="\\lambda"}
是用于加权的超参数，控制了TD和MC的折中。

- **优点**：
  - 在方差和偏差之间取得了更好的折中。通过调节 [\\lambda]{.ztext-math
    eeimg="1" tex="\\lambda"} ，GAE可以灵活控制偏差和方差之间的权衡。
- **缺点**：
...
- **GAE**则通过平衡这两者的优缺点，调整 [\\lambda]{.ztext-math eeimg="1"
  tex="\\lambda"}
  来控制估计中的偏差和方差，从而获得更稳定、更准确的估计。当 [\\lambda =
  0]{.ztext-math eeimg="1" tex="\\lambda = 0"} 时，GAE等价于TD(0)，当
  [\\lambda = 1]{.ztext-math eeimg="1" tex="\\lambda = 1"}
  时，GAE等价于MC方法。
...
- **GAE方法**：折中，既能减少偏差，也能降低方差，通过调整
  [\\lambda]{.ztext-math eeimg="1" tex="\\lambda"} 来灵活控制。
...
[\\langle S, A, P, R, \\gamma \\rangle]{.ztext-math eeimg="1"
tex="\\langle S, A, P, R, \\gamma \\rangle"} 表示，其中：

• [S]{.ztext-math eeimg="1" tex="S"} ：状态空间

• [A]{.ztext-math eeimg="1" tex="A"} ：动作空间

• [P(s{\\prime} \\mid s,a)]{.ztext-math eeimg="1"
tex="P(s{\\prime} \\mid s,a)"} ：状态转移概率，描述在状态
[s]{.ztext-math eeimg="1" tex="s"} 执行动作 [a]{.ztext-math eeimg="1"
tex="a"} 后转移到下一个状态 [s{\\prime}]{.ztext-math eeimg="1"
tex="s{\\prime}"} 的概率

• [R(s,a)]{.ztext-math eeimg="1" tex="R(s,a)"}
：即时奖励函数，衡量在状态 [s]{.ztext-math eeimg="1" tex="s"} 执行动作
[a]{.ztext-math eeimg="1" tex="a"} 所得到的回报

• [\\gamma]{.ztext-math eeimg="1" tex="\\gamma"}
：折扣因子，用于平衡当前奖励和未来奖励
...
• **Actor**：输出策略 [ \\pi\_\\theta(a\|s)]{.ztext-math eeimg="1"
tex=" \\pi_\\theta(a|s)"} ；

• **Critic**：估计价值函数 [V_w(s) ]{.ztext-math eeimg="1"
tex="V_w(s) "} 或 [Q_w(s,a)]{.ztext-math eeimg="1" tex="Q_w(s,a)"} ；
...
- DQN 中的经验回放池就是典型的 off-policy：采集数据时使用的是
  [\\epsilon-greedy]{.ztext-math eeimg="1" tex="\\epsilon-greedy"}
...
重要性采样是一种统计方法，**用来在目标分布** [p(x)]{.ztext-math
eeimg="1" tex="p(x)"} **与采样分布** [q(x)]{.ztext-math eeimg="1"
tex="q(x)"} **不一致的情况下，调整采样结果的权重，使得从**
[q(x)]{.ztext-math eeimg="1" tex="q(x)"} **中采样的数据可以用于估计**
[p(x)]{.ztext-math eeimg="1" tex="p(x)"} **的期望。【10】**

假设我们希望计算目标分布[p(x)]{.ztext-math eeimg="1" tex="p(x)"}
下某个函数 [f(x)]{.ztext-math eeimg="1" tex="f(x)"} 的期望：

[\\mathbb{E}\_p\[f(x)\] = \\int f(x)p(x) \\, dx]{.ztext-math eeimg="1"
tex="\\mathbb{E}_p[f(x)] = \\int f(x)p(x) \\, dx"}

如果[p(x)]{.ztext-math eeimg="1"
tex="p(x)"}很难直接采样，但可以从一个容易采样的分布 [q(x)]{.ztext-math
eeimg="1" tex="q(x)"} 中采样，则可以通过以下公式重写：

[\\mathbb{E}\_p\[f(x)\] = \\int f(x) \\frac{p(x)}{q(x)} q(x) \\,
dx]{.ztext-math eeimg="1"
tex="\\mathbb{E}_p[f(x)] = \\int f(x) \\frac{p(x)}{q(x)} q(x) \\, dx"}

其中， [\\frac{p(x)}{q(x)}]{.ztext-math eeimg="1"
tex="\\frac{p(x)}{q(x)}"} 称为**重要性权重（Importance Weight）**。
...
- 提高采样效率，尤其在[p(x)]{.ztext-math eeimg="1"
  tex="p(x)"}较难采样的情况下；
...
对于两个对象 [i]{.ztext-math eeimg="1" tex="i"} 和 [j]{.ztext-math
eeimg="1" tex="j"} ，模型定义对象 [i]{.ztext-math eeimg="1" tex="i"}
被偏好的概率为：

[P(i \\succ j) = \\frac{\\exp(\\beta_i)}{\\exp(\\beta_i) +
\\exp(\\beta_j)}]{.ztext-math eeimg="1"
tex="P(i \\succ j) = \\frac{\\exp(\\beta_i)}{\\exp(\\beta_i) + \\exp(\\beta_j)}"}

其中：

• [ \\beta_i]{.ztext-math eeimg="1" tex=" \\beta_i"} 和
[\\beta_j]{.ztext-math eeimg="1" tex="\\beta_j"} 分别是对象
[i]{.ztext-math eeimg="1" tex="i"} 和 [j]{.ztext-math eeimg="1" tex="j"}
的潜在得分（latent score）。
...
- **特点**
  ...
  - 更新时使用 [\\log \\pi\_\\theta(a_t \\mid s_t)]{.ztext-math
    eeimg="1" tex="\\log \\pi_\\theta(a_t \\mid s_t)"}
    乘以当时获得的总回报，获取策略梯度。
- **优点**
...
[R(\\text{回答})]{.ztext-math eeimg="1" tex="R(\\text{回答})"}
；对同一问题的回答A和回答B，若人类偏好A，则训练目标是 [R(A) \>
R(B)]{.ztext-math eeimg="1" tex="R(A) > R(B)"} ，常用对数似然形式的
[\\sigma(R(A) - R(B))]{.ztext-math eeimg="1" tex="\\sigma(R(A) - R(B))"}
来表示。
...
V(s)]{.ztext-math eeimg="1" tex="Advantage(s, a) = Q(s, a) - V(s)"}

其中， [Advantage(s, a)]{.ztext-math eeimg="1" tex="Advantage(s, a)"}
表示在状态 [s]{.ztext-math eeimg="1" tex="s"} 下采取动作 [a]{.ztext-math
eeimg="1" tex="a"} 的优势函数值， [Q(s, a)]{.ztext-math eeimg="1"
tex="Q(s, a)"} 表示状态动作对 [(s, a)]{.ztext-math eeimg="1"
tex="(s, a)"} 的动作值函数（也称为动作优势函数）， [V(s)]{.ztext-math
eeimg="1" tex="V(s)"} 表示状态值函数。
...
[\\lambda]{.ztext-math eeimg="1" tex="\\lambda"} **越接近
...
s_t)}{\\pi\_{\\text{old}}(y_t \\mid s_t)}]{.ztext-math eeimg="1"
tex="\\frac{\\pi_\\theta(y_t \\mid s_t)}{\\pi_{\\text{old}}(y_t \\mid s_t)}"}
。其中， [\\pi\_\\theta(y_t \\mid s_t)]{.ztext-math eeimg="1"
tex="\\pi_\\theta(y_t \\mid s_t)"} 和 [\\pi\_{\\text{old}}(y_t \\mid
s_t)]{.ztext-math eeimg="1" tex="\\pi_{\\text{old}}(y_t \\mid s_t)"}
分别表示新旧策略， [y_t]{.ztext-math eeimg="1" tex="y_t"} 和
[s_t]{.ztext-math eeimg="1" tex="s_t"}
...
[\\epsilon]{.ztext-math eeimg="1" tex="\\epsilon"}
...
[r(s\_{t}, a\_{t}) = I(s\_{t}=\[EOS\]) r(x, y)-\\beta KL(t)]{.ztext-math
eeimg="1" tex="r(s_{t}, a_{t}) = I(s_{t}=[EOS]) r(x, y)-\\beta KL(t)"}
，其中

[KL(t)=log (\\frac{\\pi\_{\\theta\_{old }}\^{RL}(a\_{t} \|
s\_{t})}{\\pi\^{SFT}(a\_{t} \| s\_{t})})]{.ztext-math eeimg="1"
tex="KL(t)=log (\\frac{\\pi_{\\theta_{old }}^{RL}(a_{t} | s_{t})}{\\pi^{SFT}(a_{t} | s_{t})})"}
...
\\hat{A}\_{t})\]]{.ztext-math eeimg="1"
tex="L^{CLIP}(\\theta)=\\mathbb{E}_{t}[min (r_{t}(\\theta) \\hat{A}_{t}, clip(r_{t}(\\theta), 1-\\epsilon, 1+\\epsilon) \\hat{A}_{t})]"}
，

将概率比率 [r\_{t}(\\theta)]{.ztext-math eeimg="1" tex="r_{t}(\\theta)"}
限制在 [\[1-\\epsilon, 1+\\epsilon\]]{.ztext-math eeimg="1"
tex="[1-\\epsilon, 1+\\epsilon]"} 范围内（通常 [\\epsilon =
0.2]{.ztext-math eeimg="1" tex="\\epsilon = 0.2"}
...
\\cdot \\sum\_{i=t}\^{T} KL(i)]{.ztext-math eeimg="1"
tex="A_{t}(s_{t}, a_{t}) = r(x, y)-\\beta \\cdot \\sum_{i=t}^{T} KL(i)"}
，并使用z score归一化，即 [A\_{normalized
}=\\frac{A-\\mu\_{A}}{\\sigma\_{A}}]{.ztext-math eeimg="1"
tex="A_{normalized }=\\frac{A-\\mu_{A}}{\\sigma_{A}}"}

（ [\\mu\_{A}]{.ztext-math eeimg="1" tex="\\mu_{A}"} 和
[\\sigma\_{A}]{.ztext-math eeimg="1" tex="\\sigma_{A}"}
